# main.py — FINAL US + HK DUAL MARKET (Latest loosened criteria, correct text)
import os
...
def pretty_table(df):
    if df.empty: return "No setups today across US+HK"
    h = "┌─────┬───┬───────┬───────┬─────┬──────┬───────┬─────┐"
    m = "├─────┼───┼───────┼───────┼─────┼──────┼───────┼─────┤"
    b = "└─────┴───┴───────┴───────┴─────┴──────┴───────┴─────┘"
...
def run_scan(trigger="Scheduled"):
    ...
    for i, t in enumerate(tickers):
        ...
        try:
            ...
            spy = yf.download(benchmark, period="3mo", progress=False)
    ...
    msg = f"*US + HONG KONG MINERVINI — {now_hk()}*\n"
    msg += f"*{len(setups)} SETUPS FOUND* (US + HK markets)\n"
    msg += f"Total exposure if taken: *{total_w:.1f}%*\n\n"
    msg += f"```{table}```\n\n"
    msg += "• Stage 2 uptrend\n• Recent outperformance\n• Volume bias (up > down)\n• Near/just past pivot (±12%)\nNo VCP · Maximum action"

    send_whatsapp(msg)
    send_whatsapp(f"US+HK SCAN COMPLETE — {len(setups)} setups ready")
...
print("US + HK DUAL MARKET BOT — LATEST LOOSENED CRITERIA — LIVE")
